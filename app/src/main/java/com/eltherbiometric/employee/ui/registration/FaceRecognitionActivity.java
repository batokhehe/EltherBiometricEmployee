...
public class FaceRecognitionActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    private NativeMethods.TrainFacesTask.Callback trainFacesTaskCallback = new NativeMethods.TrainFacesTask.Callback() {
        ...
        public void onTrainFacesComplete(boolean result) {
            if (result)
                showToast("Training complete " + nik, Toast.LENGTH_SHORT);
            else
                showToast("Training failed " + nik, Toast.LENGTH_LONG);
        }
    };
...
//        setSupportActionBar(mToolbar); // Sets the Toolbar to act as the ActionBar for this Activity window
        ...
        final Button takePicture = findViewById(R.id.take_picture_button);
...
//                addLabel(nik);
                counter++;
                if (counter < 10) {
                    takePicture.setText(String.format("Take Picture %d/10", counter));
                } else {
